...
class _CalendarWidgetState extends State<CalendarWidget> {
  ...
  Future<void> _fetchReviewCount() async {
    final count = await DatabaseHelper.countUlasan();
    setState(() {
      _reviewCount = count;
    });
  }
  ...
  Future<void> _fetchAverageRating() async {
    final average = await DatabaseHelper.averageUlasan();
    setState(() {
    ...
    return Column(
      children: [
        ...
        Center(
          child: Row(
            mainAxisAlignment: MainAxisAlignment.center,
            children: _buildStarIcons(), // Panggil fungsi untuk menampilkan ikon bintang
          ),
        ),
        SizedBox(height: 20,),
        Image.asset(
          'assets/line.png', // Pastikan path sudah sesuai dengan file image
  ...
  List<Widget> _buildStarIcons() {
    ...
    for (int i = 0; i < fullStars; i++) {
      stars.add(Icon(Icons.star, color: Colors.yellow, size: 15)); // Ukuran 15
    }

    if (hasHalfStar) {
      stars.add(Icon(Icons.star_half, color: Colors.yellow, size: 15)); // Ukuran 15
    }

    while (stars.length < 5) {
      stars.add(Icon(Icons.star_border, color: Colors.yellow, size: 15)); // Ukuran 15
    }
